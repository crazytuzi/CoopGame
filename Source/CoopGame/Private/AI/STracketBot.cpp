...
static int32 DebugTracketBotDrawing = 0;
FAutoConsoleVariableRef CVARDebugTracketBotDrawing(
	TEXT("COOP.DebugTracketBot")
	,
	DebugTracketBotDrawing
	,
	TEXT("Draw Debug Lines for TracketBot")
	,
	ECVF_Cheat
);
...
void ASTracketBot::BeginPlay()
{
	...
	if (Role == ROLE_Authority)
	{
		...
		GetWorldTimerManager().SetTimer(TimerHandle_CheckPowerLevel, this, &ASTracketBot::OnCheckNearbyBots, 1.f, true);
	}
}
...
void ASTracketBot::OnCheckNearbyBots()
{
	...
	{
		ASTracketBot* Bot = Cast<ASTracketBot>(Result.GetActor());
